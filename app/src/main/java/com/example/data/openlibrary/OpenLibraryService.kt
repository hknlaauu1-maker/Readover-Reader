...
Tam o sırada kapı açıldı ve Ruth içeri girdi. Martin hayatında hiç bu kadar zarif, tertemiz ve ışık saçan bir varlık görmemişti. O an, bu kızın dünyasına layık olabilmek için ne pahasına olursa olsun okuyacağına ve yazacağına ant içti.
    ...
    // Live search against Open Library, Gutendex (Gutenberg API) & Internet Archive
    suspend fun searchOpenBooks(query: String): List<OpenBookItem> = withContext(Dispatchers.IO) {
        val results = mutableListOf<OpenBookItem>()
        val cleanQuery = query.trim().lowercase()

        // 1. Search in curated catalog first
        val matchingCurated = curatedCatalog.filter {
            it.title.lowercase().contains(cleanQuery) ||
            it.author.lowercase().contains(cleanQuery) ||
            it.category.lowercase().contains(cleanQuery) ||
            it.description.lowercase().contains(cleanQuery)
        }
        results.addAll(matchingCurated)

        // 2. Query Gutendex (Project Gutenberg Open REST API)
        try {
            val encoded = URLEncoder.encode(query.trim(), "UTF-8")
            val url = "https://gutendex.com/books/?search=$encoded"
            val request = Request.Builder()
                .url(url)
                .header("User-Agent", "Readover-Open-Library-Client/1.0")
                .build()
                ...
                if (response.isSuccessful) {
                    ...
                    if (!body.isNullOrBlank()) {
                        ...
                        if (resultsArray != null) {
                            for (i in 0 until minOf(resultsArray.length(), 15)) {
                                val item = resultsArray.getJSONObject(i)
                                val id = "gutenberg-" + item.optInt("id", 0)
                                val title = item.optString("title", "Açık Kaynak Eser")
                                val authorsArray = item.optJSONArray("authors")
                                val authorName = if (authorsArray != null && authorsArray.length() > 0) {
                                    val authObj = authorsArray.getJSONObject(0)
                                    authObj.optString("name", "Bilinmeyen Yazar")
                                        .replace(",", "")
                                } else {
                                    "Klasik Edebiyat"
                                }
                                ...
                                val cover = formats?.optString("image/jpeg", null)
                                val textUrl = formats?.optString("text/plain; charset=utf-8")
                                    ?: formats?.optString("text/plain; charset=us-ascii")
                                    ?: formats?.optString("text/plain")
                                    ?: formats?.optString("text/html")

                                val downloadCount = item.optInt("download_count", 100)
                                val languagesArray = item.optJSONArray("languages")
                                val lang = if (languagesArray != null && languagesArray.length() > 0) {
                                    when (languagesArray.getString(0)) {
                                        "tr" -> "Türkçe"
                                        "en" -> "İngilizce"
                                        "fr" -> "Fransızca"
                                        "de" -> "Almanca"
                                        "es" -> "İspanyolca"
                                        else -> languagesArray.getString(0).uppercase()
                                    }
                                } else "Türkçe / Klasik"

                                val subjects = item.optJSONArray("subjects")
                                val category = if (subjects != null && subjects.length() > 0) {
                                    subjects.getString(0).substringBefore("--").trim()
                                } else "Dünya Edebiyatı"
                                ...
                                if (results.none { it.title.equals(title, ignoreCase = true) }) {
                                    results.add(
                                        OpenBookItem(
                                            ...
                                            format = "EPUB",
                                            source = "Project Gutenberg",
                                            description = "Project Gutenberg açık kaynak koleksiyonundan ücretsiz kamu malı edebi eser.",
                                            downloadUrl = textUrl,
            ...
            Log.d(TAG, "Gutendex query failed: ${e.message}")
        }

        // 3. If still empty, return curated catalog matching or all curated items
        if (results.isEmpty()) {
            results.addAll(curatedCatalog)
        }
